...
APP := snow-weave-etl$(BIN_SUFFIX)
GOFILES := $(shell find . -type f -name '*.go' -not -path '*/\.*')

.DEFAULT_GOAL: all
.DEFAULT: all

.PHONY: all
all: $(APP) ## Build the binary
...
.PHONY: wsl
wsl: ## Run the whitespace linter
	@wsl $$(go list ./... | grep -v -E "/pkg/proto/|/test/mock/")

.PHONY: wsl-fix
wsl-fix: ## Reformat source codes with the whitespace linter
	@wsl --fix $$(go list ./... | grep -v -E "/pkg/proto/|/test/mock/")
